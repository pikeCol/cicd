...
export class CrawlerService {
    ...
    private async crawlLinks(url: string, artist: string, path: string){
        ...
        await page.waitForSelector('a.aPPf7.HT3w5.lBssT');
        ...
        const links = await page.evaluate((artist: string) => {
            const allLinks = Array.from(document.querySelectorAll('a.aPPf7.HT3w5.lBssT'));
    ...
    for (var j = 0; j <inSongs.length; j++){
        //formats the artist and song title to the site format using regular expressions
        var re = / /gi;
        var inSong = inSongs[j][0].replace(re, '%20').toLowerCase();
